"use strict";
/* 예약목록 조회 */
const getReservationList = (keyword, category, status, space, date, begin, pageSize) => {
    // TODO 검색조건이 추가되면 변경
  ...
  `;
    let params = [keyword, status, space, date, date, begin, pageSize];
    params = params.filter(function (e) { return e === 0 || e; });
    return { query, params };
};
/* 예약하기 */
const doReservation = (title, space, room, name, password, content, startDate, endDate, salt, reservationStatus) => {
    const query = `
    INSERT INTO
      reservation
      (
...
/* 상세페이지 접근 시, 조회수 업데이트 */
const updateReserviatonViewCount = (id) => {
    const query = `
    UPDATE
      reservation
    SET
...
/* 예약상태 변경 */
const changeReservationStatus = (reservationStatus, id) => {
    const query = `
    UPDATE
      reservation
    SET
...
/* 예약 수정하기 */
const modifyReservation = (title, space, room, name, password, content, startDate, endDate, salt, id) => {
    const query = `
    UPDATE
      reservation
    SET
